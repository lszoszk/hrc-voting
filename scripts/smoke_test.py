...
with sync_playwright() as p:
    ...
    checks["ga countries: local body default label"] = pg.evaluate("document.querySelector('#c-body option[value=\"\"]').textContent")
    checks["ga countries: CHR option disabled"] = pg.evaluate("document.querySelector('#c-body option[value=CHR]').disabled")
    pg.select_option("#c-country", "TUV"); pg.wait_for_timeout(600)
    checks["ga countries: Tuvalu has votes"] = pg.evaluate("document.querySelectorAll('#c-rows .gtr').length > 100")
    pg.click('.tab[data-view="consensus"]'); pg.wait_for_timeout(500)
    checks["ga consensus: notice, no crash"] = pg.evaluate("document.getElementById('cn-tiles').textContent.includes('recorded votes only')")
    checks["ga years tile"] = pg.evaluate("[...document.querySelectorAll('#ov-tiles .tile')].map(t=>t.textContent).find(t=>/Years/i.test(t))||''")
    pg.click('.tab[data-view="overview"]'); pg.wait_for_timeout(300)
    ...
    pg.click('.tab[data-view="country"]'); pg.wait_for_timeout(400)
    pg.select_option("#c-country", "TUV"); pg.wait_for_timeout(600)
    checks["hrc countries: Tuvalu empty-state note"] = pg.evaluate("document.querySelector('#c-tiles .note') && document.querySelector('#c-tiles .note').textContent.includes('General Assembly')")
    checks["hrc countries: GA option disabled"] = pg.evaluate("document.querySelector('#c-body option[value=GA]').disabled")
